...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "roi": {
        ...
        "minimap": {"x1": 0.0, "y1": 0.0, "x2": 0.30, "y2": 0.31},
    },
    "scaling": {
        "enabled": True,
...
def locate_minimap_content_rect(bgr: np.ndarray, config: dict[str, Any]) -> Rect:
    ...
    search_h = min(scale_int(360, pixel_scale), height)
    search_w = min(scale_int(620, pixel_scale), width)
    ...
    bottom = 0
    for y in range(scale_int(250, pixel_scale), min(scale_int(330, pixel_scale), frame_mask.shape[0])):
        ...
        if len(segment) and np.count_nonzero(segment) > 0.65 * max(1, right - left):
            ...
            break
    if not bottom:
        bottom = min(top + scale_int(208, pixel_scale), height)
