...
If the system volume line flashes yellow, Windows audio is muted or below `30%`. The `IGNORE` button appears only while that warning is active; click it to suppress the current warning. While ignored, the small `WARN` button stays visible and flashes yellow until system volume recovers or you click it to re-enable the warning. If the condition lasts 3 minutes, a warning prompt appears. If an alert volume section turns red, that alert intensity is below `25%`.
